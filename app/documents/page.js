...
const DOCS = [
  { key: 'epf_form_20', title: 'EPF Form 20 — Withdrawal Claim', agency: 'EPFO', desc: 'Lump-sum Provident Fund balance of the deceased member.' },
  { key: 'epf_form_10d', title: 'EPF Form 10D — Monthly Pension', agency: 'EPFO', desc: 'Family / widow pension under EPS-1995.' },
  ...
  return (
    <main className="min-h-screen gt-gradient">
      ...
      <section className="max-w-5xl mx-auto px-5 py-6">
        <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">{t('documents.title', lang)}</h1>
        <p className="text-slate-600">{t('documents.sub', lang)}</p>
        <div className="mt-6 grid md:grid-cols-2 gap-5">
          {DOCS.map(d => (
            <div key={d.key} className="gt-card p-5">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-[#E1EFEF] to-[#C9E4DE] flex items-center justify-center">
                ...
                {sessionId && (
                  <>
                    <a href={`/api/document/${d.key}/${sessionId}`} target="_blank"
                      ...
                      <Eye className="h-3 w-3" /> {t('documents.preview', lang)}
                    </a>
                    <a href={`/api/document/${d.key}/${sessionId}`} download
                      ...
                      <Download className="h-3 w-3" /> {t('documents.download', lang)}
                    </a>
                  </>
                )}
              </div>
